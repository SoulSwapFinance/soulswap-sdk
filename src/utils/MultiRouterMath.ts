...
export function ConstantMeanParamsFromData(
  data: ArrayBuffer
): [number, number] {
...
export function ConstantMeanDataFromParams(
  weight0: number,
  weight1: number
): ArrayBuffer {
...
export function HybridgetY(pool: Pool, x: number): number {
  ...
  const A = HybridParamsFromData(pool.data)
  return calcSquareEquation(
    16 * A * x,
    16 * A * x * x + 4 * D * x - 16 * A * D * x,
    -D * D * D
  )[1]
}
...
export function calcInByOut(
  ...
  ASSERT(
    () => closeValues(amountOut, calcOutByIn(pool, input, direction), 1e-6),
    'Error 138'
  )
  return input
}
    ...
    case PoolType.ConstantMean: {
      ...
      return (
        (pool.reserve1 *
          weightRatio *
          (1 - pool.fee) *
          Math.pow(pool.reserve0 / x, weightRatio)) /
        x
      )
    }
    case PoolType.Hybrid: {
...
function calcInputByPriceConstantMean(pool: Pool, price: number) {
  ...
  const t =
    ...
    weightRatio *
    (1 - pool.fee) *
    Math.pow(pool.reserve0, weightRatio)
