...
class Edge {
    ...
    if (v === this.vert1) {
      if (this.direction) {
        if (amountIn < this.amountOutPrevious) {
          out =
            ...
            calcInByOut(pool, this.amountOutPrevious - amountIn, false)
        } else {
          out =
      ...
      if (this.direction) {
        ...
        if (amountIn < this.amountOutPrevious) {
          ...
          out =
            calcOutByIn(pool, amountIn - this.amountOutPrevious, true) +
            this.amountInPrevious
        }
      }
    }
  ...
  checkMinimalLiquidityExceededAfterSwap(
    ...
    if (from === this.vert0) {
      ...
      return (
        this.pool.reserve0 - amountOut + this.amountInPrevious <
        this.MINIMUM_LIQUIDITY
      )
    else
      return (
...
class Graph {
  ...
  topologySort(): Vertice[] {
    ...
    const sorted = Array.from(sortOp.sort().keys()).map(k =>
      nodes.get(k)
    ) as Vertice[]
